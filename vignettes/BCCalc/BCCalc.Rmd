...
# Introduction
This vignette demonstrates how to back-calculate lengths at previous ages from radial measurements made on calcified structures of fish. This vignette assumes that you have and understanding of
...
library(dplyr)     # for %>%, arrange(), group_by, summarize()
```
...
```{r}
SMBassWB <- inner_join(SMBassWB1,SMBassWB2,by="id") %>% arrange(id)
head(SMBassWB,n=3)
tail(SMBassWB,n=3)
```
