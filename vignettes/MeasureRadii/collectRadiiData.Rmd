...
After the scaling factor has been determined, you will be asked to select a transect on the structure image on which annular marks will be selected. This transect is selected by **first selecting the structure focus** and **then selecting the structure margin/edge**. If `addTransect=TRUE` (the default), then a line connecting these two points will be drawn on the structure image. The color of the transect may be changed with `col.transect=`. The width of the transect may be increased by including a number greater than 1 in `lwd.transect=`.
...
In theory all selected points should fall on the selected transect. In practice, you may select a point slightly off of the transect. The selected point can be "moved" perpendicularly to fall exactly on the transect by including `snap2Transect=TRUE`. The point plotted on the image will be the selected point, but point saved will the point adjusted to be on the transect.[^seesnappedpoints]
...
The radial measurements from one structure may be seen by submitting the RData file name to `combineData()` (which assumes that the file is in the current working directory).
...
this list needs to limited to only those files that contain the keyword "Scale".
```{r}
( fns <- listFiles("RData",other=c("Scale","DHO\\.")) )
```

The result of `listFiles()` should be saved to an object so that the list of name can be given to `combineData()` as shown below.
...
[^seesnappedpoints]: The recorded points on the transect will be seen if the results are viewed with `showDigitizedImage()`.
