...
This vignette assumes that you have a basic understanding of how to back-calculate fish lengths at previous ages as described in the [Short Introduction to Back-Calculation](../BCIntro/BCIntro.html) vignette. At the very least, you should be aware of what calcified structures and radial measurements are.
...
The working directory of R should be set to the directory that contains the images with `setwd()`. The following is an example of setting a working directory in R.
...
Finally, note that the process described herein will produce one R data object file (.rds files; hereafter called "R data file") for each set of measurements made on a structure image. These R data files will be saved in the current work directory (likely the directory with the structure image files). I suggest keeping the R data files and corresponding structure image files together at all times, as the R data files serve as archives of the data collected from the structure image files.
...
For example (*but described thoroughly further below*), the line below identifies "Scale_1.jpg" as the structure image, this fish has an identification number of "1", this reading of the image should be labelled as "DHO",[^readingex] and the structure margin or edge should not be considered as an annulus. When this line is run it will open the image,[^inwd] allow the user to select points that represent a transect and annuli on that transect, and save radial measurements to "Scale_1_DHO.rds".[^saveex1]
...
* `img`: File name (or names; see the [Processing Multiple Images]() section) for the structure image (or images), which must be in the current working directory. A dialog box will be provided from which the image file (or files) can be selected if this argument is not given and you are on a Windows machine.
...
After the scaling factor has been determined (or provided), you will select a transect on the structure image on which annular marks will be selected. This transect is selected by **first selecting the structure focus** and **then selecting the structure margin/edge** and pressing the 'f' key.[^deletekey] If `showTransect=TRUE` (the default), then a line connecting these two points will be drawn on the structure image. The color of the transect may be changed with `col.transect=`. The width of the transect may be increased by including a number greater than 1 in `lwd.transect=`.
...
In theory all selected points should fall on the selected transect. In practice, you may select a point slightly off the transect. The selected point can be "moved" perpendicularly to fall exactly on the transect by including `snap2Transect=TRUE`, which is the default behavior.
...
When you have finished selecting points, the image window is closed[^closewindow] and information about your selections, including the calculated radial measurements, are saved to the R data file. The contents of this file are described further [here](http://derekogle.com/RFishBC/reference/digitizeRadii.html#value).

\  

### Processing Multiple Images {#multipleimages}
It will be common to process a number of image files, one after another. The selection of the images can be made more efficient by supplying `digitizeRadii()` a vector of image file names and corresponding fish IDs. These vectors can be constructued in a variety of ways before calling `digitizeRadii()`. First, the user can simply type all of the image names and fish IDs into vectors; e.g.,
```{r eval=FALSE}
imgs <- c("Scale_1.jpg","Scale_2.jpg")
ids <- c("1","2")
```
...
With these vectors, the following call to `digitizeRadii()` will bring up the first image on which you can mark annuli as described above. When you have finished with the first image then the second image will automatically appear on which you can then mark the annuli. This process will be repeated until the last image in the vector of images has been completed.
```{r eval=FALSE}
...
Default values for these arguments may be changed within `digitizeRadii()`. For example, the code below sets the "reading" label to "DHO", the edge to not be considered an annulus, the width of the transect line to be thicker, and a scale-bar is present with a known length of 0.6 mm.
```{r eval=FALSE}
digitizeRadii("Scale_1.jpg",id="1",reading="DHO",edgeIsAnnulus=FALSE,
...
The radial measurements from one structure may be seen by submitting the R data file name to `combineData()`.[^combinedatawd] Note that by default the radial measurement that includes the "plus-growth" will be omitted (as this radial measurement is equal to the total radial measurement and is thus redundant). To include the radial measurement with "plus-growth" then use `deletePlusGrowth=FALSE`.

```{r}
combineData("Scale_1_DHO.rds")
```

By default the data are shown in "long" format where each row consists of one radial measurement and all radial measurements for an individual fish are distributed across several rows. The radial data can be shown in "wide" format where each row consists of all the radial measurements (in separate columns) for an individual fish by including `outFormat="wide"`.

```{r}
combineData("Scale_1_DHO.rds",outFormat="wide")
```
...
This list of names can be further filtered by including other key words for the filenames in `other=`. In this case, the list should be limited to those files with "Scale" in the name and those files with just "DHO" (i.e., exclude "DHO2"). Limiting to "DHO" required limiting to "DHO.", but because the "." is a special character it had to be preceded by two back slashes.
```{r}
...
Again, the data can be shown in "wide" format by including `outFormat="wide"`.

```{r}
( dfrad2 <- combineData(fns,outFormat="wide") )
```
...
One could also join with the "wide" data.
```{r}
fishdat2 <- dffish %>%
...
[^deletekey]: The last selected point can be deleted by pressing the 'd' or 'r' keys. This can be done multiple times such that the last number of selections can be deleted. The deleted points will be marked with the plotting character and color in `pch.del=` and `col.del=` (defaults to a red circle with an 'x' in it).
...
[^windowsize]: Strictly, `windowSize=` only works if `sepWindow=TRUE`, which is the default as I have found it "safer" to open images in their own window (and separate from the RStudio plot pane).
...
[^closewindow]: The image window is closed by default when "f" is pushed after the last annulus is selected. The window may be kept open by using `closeWindow=FALSE` in the call to `digitizeRadii()`.
